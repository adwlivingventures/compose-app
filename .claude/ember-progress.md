...
- **Differentiate the free preview from paid Phase IV content** (when E19 lands / before real users reach Day 76): the Autonomic Sync preview (`app/autonomic-sync.tsx`) overlaps the paid tier twice — (a) its Copilot simulation shares scenario 1's reframe nearly verbatim with `app/copilot.tsx`, and (b) its lesson cards 1–2 ("The Polyvagal Secret"/"The Anxiety Transfer") are near-verbatim Sensate Mastery steps 1–2 (`app/sensate-mastery.tsx` — the Anxiety Transfer body is essentially identical). Correct while the preview is a sample, but a paying Day-76 user shouldn't feel he already got the product. Fix by trimming the preview to true teaser depth and/or expanding the paid modules' content beyond the preview's.
