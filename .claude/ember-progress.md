...
- **Differentiate the Copilot preview from the paid Copilot** (when E19 lands / before real users reach Day 76): the Autonomic Sync preview (`app/autonomic-sync.tsx`) and the maintenance-tier Somatic Copilot (`app/copilot.tsx`) currently share scenario 1's reframe nearly verbatim — correct while the preview is a sample, but a paying Day-76 user shouldn't feel he already got the product. Either give the preview a shortened/teaser variant of the response, or expand the paid version's scenario set beyond the three in the preview.
